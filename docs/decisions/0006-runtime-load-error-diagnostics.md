...
対象 workflow と、任意で ComfyUI frontend Error Report を指定して、以下を同じ report にまとめる。
...
4. ComfyUI frontend Error Report から deterministic parser で作った正規化 event
...
frontend Error Report も raw text として LLM に渡さず、deterministic parser で section を分解する。

```text
ComfyUI Error Report markdown
  -> section parser
  -> stack frame parser
  -> redactor
  -> event classifier
...
frontend Error Report 由来の event は以下の形を許す。
...
- LLM / agent には raw log や raw Error Report ではなく、正規化済み event と compact report だけを渡す。
